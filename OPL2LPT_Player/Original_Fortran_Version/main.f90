...
module VGMPlayer
    ...
    subroutine getProcPointer()
        ...
        IF (.NOT. C_ASSOCIATED(proc_address3)) STOP 'Unable to obtain procedure address'
...
!    integer(kind = 16) :: YM3812clock, YM3526Clock
    integer(kind = 8) :: YM3812clock, YM3526Clock
    character(len=6) :: IC
    ...
    !call execute_command_line("apps\drv\InstallDriver.exe", wait=.FALSE.)
